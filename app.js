...
function fillBakerSelect(select, bakers, { blankLabel = "Choose baker", selectedId = "" } = {}) {
  ...
  for (const baker of bakers) {
    select.appendChild(option(baker.id, baker.name, baker.id === selectedId));
  }
}
...
function renderBakerList() {
  ...
  el.innerHTML = `
    <table>
      <thead><tr><th>Baker</th><th>Status</th></tr></thead>
      <tbody>
        ${state.bakers.map((b) => `<tr><td>${escapeHtml(b.name)}</td><td>${b.is_active ? "Active" : "Eliminated"}</td></tr>`).join("")}
      </tbody>
    </table>
  `;
}
...
async function saveResults(event) {
  ...
  try {
    ...
    const resultRes = await state.supabase
      .from("results")
    ...
    if (selectedHandshakeIds.length) {
      const insertRes = await state.supabase.from("result_handshakes").insert(
        selectedHandshakeIds.map((baker_id) => ({ result_id: resultId, baker_id }))
      );
      if (insertRes.error) throw insertRes.error;
    }

    if (eliminatedId) {
      const elimRes = await state.supabase
        .from("bakers")
...
async function renderLeaderboard() {
  ...
  try {
    ...
    lb.innerHTML = rows.length ? `
      <table>
        <thead><tr><th>Position</th><th>Player</th><th>Points</th></tr></thead>
        <tbody>
          ${rows.map((r, i) => `<tr><td>${i + 1}</td><td>${escapeHtml(r.player_name)}</td><td>${r.total_points}</td></tr>`).join("")}
        </tbody>
      </table>
    ...
    const predictionsRes = await state.supabase
      ...
      .select(`
        id,
        players(name),
        weeks(week_number,title),
        technical:bakers!predictions_technical_winner_baker_id_fkey(name),
        star:bakers!predictions_star_baker_id_fkey(name),
        eliminated:bakers!predictions_eliminated_baker_id_fkey(name),
        handshake:bakers!predictions_handshake_baker_id_fkey(name)
      `)
    ...
    all.innerHTML = picks.length ? `
      <table>
        <thead><tr><th>Player</th><th>Week</th><th>Technical</th><th>Star baker</th><th>Eliminated</th><th>Handshake</th></tr></thead>
        <tbody>
          ${picks.map((p) => `
            <tr>
              <td>${escapeHtml(p.players?.name || "")}</td>
              <td>Week ${p.weeks?.week_number || ""} - ${escapeHtml(p.weeks?.title || "")}</td>
              <td>${escapeHtml(p.technical?.name || "")}</td>
              <td>${escapeHtml(p.star?.name || "")}</td>
              <td>${escapeHtml(p.eliminated?.name || "")}</td>
              <td>${escapeHtml(p.handshake?.name || "No guess")}</td>
            </tr>
          `).join("")}
        </tbody>
      </table>
...
async function handleLogin(event) {
  ...
  if (error) {
    setText("loginStatus", error.message, true);
    return;
  }
  await startApp();
}
...
async function init() {
  ...
  state.supabase = window.supabase.createClient(
    window.BAKEOFF_SUPABASE_URL,
    window.BAKEOFF_SUPABASE_ANON_KEY
  );
